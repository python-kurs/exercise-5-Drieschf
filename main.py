...
data_sel = data.sel(latitude = slice(30,72), longitude = slice(-13,25), time = slice ("1981-01-01","2010-12-31"))
mon_per = data_sel.groupby("time.month").mean("time")
...
data_18 = data.sel(latitude =slice(30,72), longitude = slice(-13,25), time = slice ("2018-01-01","2018-12-31"))
mon_18 = data_18.groupby("time.month").mean("time")

mon_anom = mon_18 - mon_per 

Plots = mon_anom["tg"].plot(x="longitude", col="month")
...
mean_anom = mon_anom.mean()["tg"]
anom_mr = mon_anom.sel(latitude = 50.80, longitude = 8.77, method = "nearest").mean()
if anom_mr["tg"] > mean_anom:
...
mon_anom.to_netcdf(output_dir / "europe_anom_2018.nc")
mr_2018 = mon_anom.sel(latitude= 50.80, longitude=8.77, method="nearest").to_dataframe()["tg"]
mr_2018.to_csv(output_dir / "marburg_anom_2018.csv")
